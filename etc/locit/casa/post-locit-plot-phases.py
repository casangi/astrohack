def _get_phases(msnam, ant, ref):
    ...
    table_obj = ctables.table(
        msnam, readonly=True, lockoptions={"option": "usernoread"}, ack=False
    )
...
def _plot_single_phase(ax, time, raw, cor, flag, corr):
    ...
    cor_rms = np.std(cor)
    mksz = 0.2
    ax.plot(time, raw, label="Raw", color="black", ls="", marker="o", markersize=mksz)
    ax.plot(
        time, cor, label="Corrected", color="red", ls="", marker="o", markersize=mksz
    )
