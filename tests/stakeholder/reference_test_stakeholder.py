...
def set_data(tmp_path_factory):
    ...
    astrohack.toolviper.utils.data.download(
        "ea25_cal_small_before_fixed.split.ms", folder=str(data_dir)
    )
    astrohack.toolviper.utils.data.download(
        "ea25_cal_small_after_fixed.split.ms", folder=str(data_dir)
    )
    astrohack.toolviper.utils.data.download(
    ...
    astrohack.toolviper.utils.data.download(file="alma-test", folder=str(data_dir))
    astrohack.toolviper.utils.data.download(file="vla-test", folder=str(data_dir))
...
def verify_panel_shifts(
    ...
    delta_shift = (
        delta_mean_shift - delta_ref_shift
...
def verify_center_pixels(
    ...
    for i in range(len(aperture_ref)):
        ...
        aperture_check = round(aperture_ref[i].real, number_of_digits) == round(
            aperture_center_pixels[i].real, number_of_digits
        )
        beam_check = round(beam_ref[i].real, number_of_digits) == round(
            beam_center_pixels[i].real, number_of_digits
        )

        real_check = aperture_check and beam_check

        aperture_check = round(aperture_ref[i].imag, number_of_digits) == round(
            aperture_center_pixels[i].imag, number_of_digits
        )
        beam_check = round(beam_ref[i].imag, number_of_digits) == round(
            beam_center_pixels[i].imag, number_of_digits
        )
...
def verify_holog_diagnostics(cell_size, grid_size, number_of_digits=7):
    ...
    json_data["cell_size"] = np.array(
        [round(x, number_of_digits) for x in json_data["cell_size"]]
    )

    return np.all(json_data["cell_size"] == cell_size) and np.all(
