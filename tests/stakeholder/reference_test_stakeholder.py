...
def set_data(tmp_path_factory):
    ...
    # Data files
    astrohack.toolviper.utils.data.download('ea25_cal_small_before_fixed.split.ms', folder=str(data_dir))
    astrohack.toolviper.utils.data.download('ea25_cal_small_after_fixed.split.ms', folder=str(data_dir))
...
def verify_center_pixels(file, antenna, ddi, reference_center_pixels, number_of_digits=7):
    ...
    for i in range(len(aperture_ref)):
        ...
        aperture_check = round(aperture_ref[i].real, number_of_digits) == round(aperture_center_pixels[i].real,
                                                                                number_of_digits)
        ...
        aperture_check = round(aperture_ref[i].imag, number_of_digits) == round(aperture_center_pixels[i].imag,
                                                                                number_of_digits)
