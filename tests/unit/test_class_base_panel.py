...
class TestBasePanel:
    ...
    def test_mean_model(self):
        ...
        meanpanel.solve()
        assert (
            abs(meanpanel.model.parameters[0] - expectedmean) / expectedmean
            < self.tolerance
        ), ("Did not recover the" " expected mean")
        meanpanel.get_corrections()
        assert (
            len(meanpanel.corr) == nsamp
        ), "Number of corrected points do not match number of samples"
        onecorr = meanpanel.model.correct_point(PanelPoint(0, 0))
        assert abs(onecorr - expectedmean) / expectedmean < self.tolerance, (
        ...
        for screw in mmscrews:
            assert abs(screw - fac * expectedmean) < self.tolerance, (
                "mm screw adjustments not within 0.1% tolerance "
                "of the expected value"
            )
        miscrews = meanpanel.export_screws(unit="mils")
        fac = convert_unit("m", "mils", "length")
        for screw in miscrews:
            assert abs(screw - fac * expectedmean) < 1e-2, (
                "Miliinches screw adjustments not " "within 1% of the expected value"
            )
    ...
    def test_rigid_model(self):
        ...
        rigidpanel.solve()
        for ipar in range(3):
            feedback = "{0:d}-eth parameter does not match its expected value".format(
                ipar
            )
            assert (
                abs(rigidpanel.model.parameters[ipar] - expectedpar[ipar])
                / abs(expectedpar[ipar])
                < self.tolerance
            ), feedback
        rigidpanel.get_corrections()
        assert (
            len(rigidpanel.corr) == nside**2
        ), "Number of corrected points do not match number of samples"
        onecorr = rigidpanel.model.correct_point(PanelPoint(0, 0))
        assert abs(onecorr - expectedpar[2]) / expectedpar[2] < self.tolerance, (
            "Correction for a point did not match " "the expected value"
        )
        mmscrews = rigidpanel.export_screws()
        fac = convert_unit("m", "mm", "length")
        for screw in mmscrews:
            assert abs(screw - fac * expectedpar[2]) < self.tolerance, (
                "mm screw adjustments not within 0.1% "
    ...
    def test_xyparaboloid_scipy_model(self):
        ...
        for ipar in range(3):
            ...
            assert (
                abs(xyparapanel.model.parameters[ipar] - expectedpar[ipar])
                / abs(expectedpar[ipar])
                < self.tolerance
            ), feedback
        xyparapanel.get_corrections()
        assert (
            len(xyparapanel.corr) == nside**2
        ), "Number of corrected points do not match number of samples"
    ...
    def test_rotatedparaboloid_scipy_model(self):
        ...
        for ipar in range(3):
            ...
            assert (
                abs(rotparapanel.model.parameters[ipar] - expectedpar[ipar])
                / abs(expectedpar[ipar])
                < self.tolerance
            ), feedback
        rotparapanel.get_corrections()
        assert (
            len(rotparapanel.corr) == nside**2
        ), "Number of corrected points do not match number of samples"
        ...
        fac = convert_unit("m", "mm", "length")
        for screw in mmscrews:
            assert abs(screw - fac * expectedpar[2]) < 1e3 * self.tolerance, (
                "mm screw adjustments not within 0.1% "
                "tolerance of the expected value"
            )
    ...
    def test_corotatedparaboloid_scipy_model(self):
        ...
        for ipar in range(3):
            ...
            assert (
                abs(corotparapanel.model.parameters[ipar] - expectedpar[ipar])
                / abs(expectedpar[ipar])
                < self.tolerance
            ), feedback
        corotparapanel.get_corrections()
        assert (
            len(corotparapanel.corr) == nside**2
        ), "Number of corrected points do not match number of samples"
    ...
    def test_fullparaboloid_lst_model(self):
        ...
        for ix in range(nside):
            for iy in range(nside):
                ...
                ysq = iy**2
                value = (
                    expectedpar[0] * xsq * ysq
                    + expectedpar[1] * xsq * iy
                    + expectedpar[2] * ysq * ix
                )
                value += (
                    expectedpar[3] * xsq
                    + expectedpar[4] * ysq
        ...
        for ipar in range(len(expectedpar)):
            ...
            assert (
                abs(fullpara.model.parameters[ipar] - expectedpar[ipar])
                / abs(expectedpar[ipar])
                < self.tolerance
            ), feedback
        fullpara.get_corrections()
        assert (
            len(fullpara.corr) == nside**2
        ), "Number of corrected points do not match number of samples"
        ...
        for screw in mmscrews:
            assert abs(screw - expectedscrew) < 1e3 * self.tolerance, (
                "mm screw adjustments not within 0.1% "
                "tolerance of the expected value"
            )
