...
class TestGridAlgorithms:
    ...
    def test_grid_linear_beam_no_chan_average(self):
        ...
        ), "Beam grid does not have the expected shape"

        for i_pol, pol in enumerate(pol_axis):
            assert np.isclose(
                beam_grid[0, 32, i_pol, 14, 14], ref_center_values[i_pol]
        ...
        ), "Grid size and l axis size are not equal"
        assert (
            m_axis.shape[0] == self.grid_size[1]
        ), "Grid size and m axis size are not equal"
        assert np.all(
            np.isclose(ref_freq_axis, output_freq_axis)
        ), "Output frequency axis is not equal to the reference"
        assert (
            not grid_corr
        ), "Linear beam griddind does not warrant a gridding correction in aperture plane"
    ...
    def test_grid_linear_beam_chan_average(self):
        ...
        for i_pol, pol in enumerate(pol_axis):
            assert np.isclose(
                beam_grid[0, 0, i_pol, 14, 14], ref_center_values[i_pol]
            ), f"Center pixel for {pol} does not match reference"
    ...
    def test_1d_linear_gridding(self):
        ...
        resamp_over, weights_over = grid_1d_data(
            dest_x_over,
            orig_x_axis,
            y_data,
            "linear",
        ...
        assert np.allclose(
            resamp_over, fill_value
        ), "Resampled data does not have the expected values"
        assert np.allclose(weights_over, 1.0), "Weights should be 1 everywhere"

        resamp_under, weights_under = grid_1d_data(
            dest_x_under,
            orig_x_axis,
            y_data,
            "linear",
        ...
        assert np.allclose(
            resamp_under, fill_value
        ), "Resampled data does not have the expected values"
        assert np.allclose(
            np.unique(weights_under), [1.0, 2.0, 3.0]
        ), "Weights should be 1, 2 or 3 everywhere"
        ...
        assert (
            n_nan == 0
        ), "There should be no NaNs when falling back to gaussian interpolation"
    ...
    def test_1d_gaussian_gridding(self):
        ...
        reference_weights = [0.00390625, 0.0078125, 1.0, 1.0]
        assert np.allclose(
            resamp_over, fill_value
        ), "Resampled data does not have the expected values"
        assert np.allclose(
            np.unique(weights_over), reference_weights, atol=1e-8
        ), "weights are not what is expected"
        ...
        assert np.allclose(
            np.unique(weights_under), reference_weights, atol=1e-8
        ), "weights are not what is expected"
