...
class ZernikeCoefficients():
    ...
    def _stok_num_to_str(self, stok_num):
        ...
        stokes_map = {0: 'Undefined',
                      ...
                      29: 'Plinear', 30: 'PFtotal', 31: 'PFlinear', 32: 'PAngle'}
    ...
    def csv_to_zarr(self, infile, outfile='', xds_attrs = {}):
        ...
        coords = {'zernike_index': coeffs, 'polarization': stok_str, 'frequency': freqs, 'antenna_index':ants}
        ds = ds.assign_coords(coords)

        # Shape coefficients appropriately
        coeff_arr = np.zeros((len(ants), len(freqs), len(stok_str), len(coeffs)), dtype=complex)
        eta_arr_p = np.zeros((len(ants), len(freqs), len(stok_str), len(coeffs)), dtype=float)
        eta_arr_q = np.zeros((len(ants), len(freqs), len(stok_str), len(coeffs)), dtype=float)
            ...
            for freqidx, freq in enumerate(freqs):
                for stokidx, stok in enumerate(stok_num):
                    for coeffidx, coeff in enumerate(coeffs):
                        ...
                        if 'antenna' in df.columns:
                            tdf = df[(df['antenna'] == ant) & (df['freq'] == freq*1e3) & (df['stokes'] == stok) & (df['ind'] == coeff)]
                        else:
                            tdf = df[(df['freq'] == freq*1e3) & (df['stokes'] == stok) & (df['ind'] == coeff)]

                        coeff_arr[antidx, freqidx, stokidx, coeffidx] = tdf['real'].values[0] + 1j*tdf['imag'].values[0]
                        eta_arr_p[antidx, freqidx, stokidx, coeffidx] = tdf['etax'].values[0]
                        eta_arr_q[antidx, freqidx, stokidx, coeffidx] = tdf['etay'].values[0]

        ds['coefficients'] = xr.DataArray(coeff_arr, dims=['antenna_index', 'frequency', 'polarization', 'zernike_index'], coords=coords)
        ds['eta_p'] = xr.DataArray(eta_arr_p, dims=['antenna_index', 'frequency', 'polarization', 'zernike_index'], coords=coords)
        ds['eta_q'] = xr.DataArray(eta_arr_q, dims=['antenna_index', 'frequency', 'polarization', 'zernike_index'], coords=coords)
...
class ZernikeFitter():
    ...
    def fit_complex_aperture(self, antenna, ddi='ddi_0'):
        ...
        fit_aperture = np.zeros_like(image_mds[antenna][ddi]['AMPLITUDE'], dtype=complex)
        res_aperture = np.zeros_like(image_mds[antenna][ddi]['AMPLITUDE'], dtype=complex)
        ...
        for pp in range(1):
            ...
            coeffsreal = leastsq(self.fit_minimize,np.ones(self.ntermsreal),
                        args=(aperture_amp, xx, yy, maskidx))[0]
            coeffsimag = leastsq(self.fit_minimize,np.ones(self.ntermsimag),
                        args=(aperture_phase, xx, yy, maskidx))[0]
            ...
            res_aperture[0,0,pp,:,:] = aperture_complex - fit_aperture[0,0,pp,:,:]
