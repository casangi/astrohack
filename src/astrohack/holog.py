...
def _calculate_aperture_pattern(grid, frequency, delta, padding_factor=50):
    ...
    print('grid.shape',grid.shape,padding)
    ...
    for avg_index in avg_chan_index:
        ...
        for i_t in range(n_time):
            for i_p in range(n_pol):
                if weight_sum[i_t, avg_index, i_p] == 0:
                    data_avg[i_t, avg_index, i_p] = 0.0
                else:
                    data_avg[i_t, avg_index, i_p] = (
                        data_avg[i_t, avg_index, i_p] / weight_sum[i_t, avg_index, i_p]
                    )
...
def _holog_chunk(holog_chunk_params):
    ...
    for ddi_index, ddi in enumerate(ant_data_dict.keys()):
        ...
        scan0 = list(ant_data_dict[ddi].keys())[0]  # For a fixed ddi the frequency axis should not change over scans, consequently we only have to consider the first scan.
        ...
        if holog_chunk_params["chan_average"]:
            ...
            beam_grid = np.zeros(
                (n_scan,) + (1, n_pol) + grid_l.shape, dtype=np.complex
            )  # Only a single channel left after averaging.
            
            
        else:
            beam_grid = np.zeros(
                (n_scan,) + (n_chan, n_pol) + grid_l.shape, dtype=np.complex
            )
        ...
        for scan_index, scan in enumerate(ant_data_dict[ddi].keys()):
            ...
            if holog_chunk_params["chan_average"]:
                ...
                for i_c in range(n_chan):  # Unavoidable for loop because lm chnage over frequency.
                    # Average scaled beams.
                    beam_grid[scan_index, 0, :, :, :] = (
                        beam_grid[scan_index, 0, :, :, :]
                        + np.moveaxis(griddata(
                            lm_freq_scaled[:, :, i_c],
                            vis_avg[:, i_c, :],
                            (grid_l, grid_m),
                ...
                beam_grid[scan_index, ...] = np.moveaxis(griddata(
                    lm,
                    vis,
                    (grid_l, grid_m),
                    method=holog_chunk_params["grid_interpolation_mode"],
            ...
            #Normalization
            for chan in range(n_chan): ###To Do: Vectorize scan and channel axis
        ...
        if holog_chunk_params["scan_average"]:
            ###To Do: Add derotation
          
            beam_grid = np.mean(beam_grid,axis=0)[None,...]
            
        print("beam grid shape", beam_grid.shape)
