...
def imshow_plot(
    ax, fig, title, x_axis, y_axis, gridded_data, cmap, minmax, fsize=5, flipxy=True
):
    if flipxy:
...
def cropped_secondary_mesh(pr_reflec, pr_pnt, sc_pnt, sc_cropped_mesh, sc_cropped_mesh_norm, sc_n_triangles):
    ...
    for ipnt in range(niter):
        ...
        if n_tri == 0:
            ...
            itriangle, sc_point = jitted_triangle_find(
                pr_point, pr_reflection, mesh_section, sc_pnt
            )
            if itriangle == intblankval:
                ...
                sc_reflec[ipnt] = reflect_on_surface(
                    pr_reflection, sc_cropped_mesh_norm[ipnt, itriangle]
                )
...
class NgvlaRayTracer:
    ...
    def primary_reflection(self):
        ...
        self.pr_reflec = (
            light - 2 * inner_product_2d(light, self.pr_norm) * self.pr_norm
        )
    ...
    def secondary_reflection_obsolete(self):
        ...
        for (
            ...
            dist_matrix = np.sqrt(
                inner_product_2d(dist_vec, dist_vec, keep_shape=False)
            )
            ...
            self.sc_reflec[it] = (
                pnt_reflec
                - 2
                * np.inner(pnt_reflec, self.sc_norm[isec_loc])
                * self.sc_norm[isec_loc]
            )
        print()
    ...
    def _grid_with_griddata(self, data_array, resolution, label):
        ...
        if np.sum(selection) == 0:
            ...
            return None
        gridded_array = griddata(
            (x_pnt[selection], y_pnt[selection]),
            data_array[selection],
            (x_mesh, y_mesh),
            "linear",
        )
    ...
    def compare_phase_gridding(
        ...
        start = time.time()
        griddata_phase, x_axis, y_axis = self._grid_with_griddata(
            self.phase, resolution, "phase"
        )
        stop = time.time()
        print(f"Gridding with griddata took {stop - start} seconds")

        start = time.time()
        lndi_phase, x_axis, y_axis = self._grid_with_lndi(
            self.phase, resolution, "phase"
        )
        stop = time.time()
        ...
        close_figure(
            fig,
            "Phase gridding comparison",
            f"phase_comparison_{resolution:.2}.png",
            300,
            False,
        )
    ...
    def plot_simple_2d(
        self, data_types, rootname, resolution, resolution_unit, colormap="viridis"
    ):
                ...
                if isinstance(item, np.ndarray):
                    if key in ["sc_pnt", "sc_norm"]:
                        ...
                        xds[key] = xr.DataArray(
                            item, dims=["pr_pnt", "crop_tri", "tri_corners"]
                        )
                    elif key in ["sc_cropped_mesh_norm"]:
                        xds[key] = xr.DataArray(
                            item, dims=["pr_pnt", "crop_tri", "xyz"]
                        )
                    else:
    ...
    def secondary_reflection_on_mesh_brute_force(self, short_circuit=False):
        ...
        for ipnt in range(niter):
            ...
            itriangle, sc_point = self._find_triangle_on_secondary(
                pr_point, pr_reflection
            )
            ...
            if np.isnan(itriangle):
                ...
                self.sc_reflec[ipnt] = reflect_on_surface(
                    pr_reflection, self.sc_mesh_norm[itriangle]
                )
            print(f"\033[FMesh reflections: {100 * ipnt / niter:.2f}%")

        self.sc_reflec_triangle = np.where(
            np.abs(self.sc_reflec_triangle - intblankval) < 1e-7,
            np.nan,
    ...
    def cropped_reflec_jit(self):
        self.sc_reflec, self.sc_reflec_pnt, self.sc_reflec_triangle = (
            cropped_secondary_mesh(
                self.pr_reflec,
                self.pr_pnt,
                self.sc_pnt,
                self.sc_cropped_mesh,
                self.sc_cropped_mesh_norm,
                self.sc_n_triangles,
            )
        )
    ...
    def secondary_to_horn(self, epsilon=1e-7):
        ...
        self.horn_intersect = np.where(
            dist_horn_mouth[:, np.newaxis] <= self.horn_diameter,
            intersect_point,
            np.nan,
        )
        return
    ...
    def compute_full_light_path(self, show_stats=False):
        ...
        self.full_light_path = (
            zeroth_distance + first_distance + second_distance + third_distance
        )
    ...
    def _plot_no_gridding(
        self, data_array, title, filename, zlim, colormap="viridis", resolution=0.1
    ):
        cmap = get_proper_color_map(colormap)
        fig, ax = create_figure_and_axes([10, 8], [1, 1])
        x_pnt = self.pr_pnt[:, 0]
    ...
    def crop_secondary_mesh(self, max_distances, i_sel=None):
        ...
        for ipnt in loop_list:
            ...
            print(
                f"{return_line}Cropping: {100 * ipnt / self.pr_pnt.shape[0]:.2f}%      "
            )
        ...
        for jpnt in range(self.pr_pnt.shape[0]):
            if jpnt == ipnt:
                ...
                try:
                    ...
                    crossings[jpnt] = (
                        self.sc_reflec_pnt[ipnt] + solution[0] * self.sc_reflec[jpnt]
                    )
                except KeyError:
                    crossings[jpnt] = nanvec3d
            else:
...
def ngvla_rt_pipeline(
    cropped_mesh_zarr_file,
    wavelength=0.007,
    incident_light=(0, 0, -1),
    focus_location=(-1.136634465810194, 0, -0.331821128650557),
    horn_orientation=(0, 0, 1),
    ...
    rtobj = NgvlaRayTracer(
        wavelength=wavelength,
        incident_light=incident_light,
        focus_location=focus_location,
        horn_orientation=horn_orientation,
        horn_length=horn_length,
        horn_diameter=horn_diameter,
        horn_position=horn_position,
...
def full_proc_pipeline(
    ...
    sc_reflec, sc_reflec_pnt, sc_reflec_triangle = cropped_secondary_mesh(
    ...
    line_par = np.where(
        np.abs(dot_horn_plane) < epsilon,
        np.nan,
        dot_horn_plane / np.dot(sc_reflec, horn_orientation),
    )
    intersect_point = sc_reflec_pnt + line_par[:, np.newaxis] * sc_reflec
    dist_horn_mouth = compute_distances(intersect_point, horn_mouth_center)
    horn_intersect = np.where(
        dist_horn_mouth[:, np.newaxis] <= horn_diameter, intersect_point, np.nan
    )
...
def actual_jitted_pipeline(
    ...
    line_par = np.where(
        np.abs(dot_horn_plane) < epsilon, np.nan, dot_horn_plane / reflec_horn
    )
    intersect_point = sc_reflec_pnt + line_par[:, np.newaxis] * sc_reflec
...
@njit(cache=True, nogil=True)
def actual_gridding(
    x_axis, y_axis, nx, ny, pr_pcd, pr_mesh, pr_mesh_norm, pr_pnt, pr_norm
):
    ...
    for ix in range(nx):
        ...
        for iy in range(ny):
            ...
            for itriangle, triangle in enumerate(pr_mesh):
                inside, intersect_pnt = moller_trumbore_algorithm(
                    ray_origin,
                    ray_vector,
                    pr_pcd[triangle[0]],
                    pr_pcd[triangle[1]],
...
def create_gridded_primary_dish(mesh_zarr_file, resolution, plot=False, raw_mesh=True):
    ...
    if plot:
        ...
        close_figure(
            fig, "Z gridded griddata", "z_griddata.png", 300, False, tight_layout=True
        )

        fig, ax = create_figure_and_axes([10, 8], [1, 1])
        scatter_plot(ax, x_pnt, "X axis (m)", y_pnt, "Y axis (m)")
        close_figure(
            fig, "Ungrided X & Y", "xy_ungridded.png", 300, False, tight_layout=True
        )
