...
def moments(
    data, circle, rotate, vheight, estimator=median, angle_guess=45.0, **kwargs
):
    """
    ...
    col = data[int(y), :]
    width_x = np.sqrt(
        np.abs((np.arange(col.size) - y) ** 2 * col).sum() / np.abs(col).sum()
    )

    row = data[:, int(x)]
    width_y = np.sqrt(
        np.abs((np.arange(row.size) - x) ** 2 * row).sum() / np.abs(row).sum()
    )
...
def gaussfit(
    ...
    minpars=np.repeat(0, 7),
    maxpars=[0, 0, 0, 0, 0, 0, 180],
    rotate=True,
    vheight=True,
    quiet=True,
    returnmp=False,
    ...
    **kwargs,
    ...
    if usemoment.any() and len(params) == len(usemoment):
        moment = np.array(
            moments(data, circle, rotate, vheight, **kwargs), dtype="float"
        )
...
def onedmoments(
    Xax, data, vheight=True, estimator=median, negamp=None, veryverbose=False, **kwargs
):
...
def onedgaussfit(
    ...
    if usemoments:
        params = onedmoments(
            xax, data, vheight=vheight, negamp=negamp, veryverbose=veryverbose
        )
        if vheight is False:
...
def n_gaussian(pars=None, a=None, dx=None, sigma=None):
    ...
    if len(pars) % 3 == 0:
        ...
        raise ValueError(
            "Wrong array lengths! dx: %i  sigma: %i  a: %i"
...
def collapse_gaussfit(
    ...
    **kwargs,
    ...
    print(
        "Fitting a total of %i spectra with peak signal above %f"
        % (
    ...
    for i in range(cube.shape[1]):
        ...
        for j in range(cube.shape[2]):
            if np.abs(extremum(cube[:, i, j])) > (mean_std * nsigcut):
                mpp, gfit, mpperr, chi2 = onedgaussfit(
                    xax,
                    cube[:, i, j],
                    err=np.ones(cube.shape[0]) * mean_std,
                    negamp=negamp,
