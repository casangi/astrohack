...
class NgvlaRayTracer:
    ...
    @classmethod
    def from_local_qps_objects(
        cls, primary_local_qps, secondary_local_qps, wavelength, telescope_parameters
    ):
    ...
    def reflect_off_primary(self, incident_light):
        ...
        self.grd_pm_reflections = reflect_light(
            self.incident_ray[np.newaxis, :], self.grd_pm_normals
        )
    ...
    def reflect_off_secondary(self):
        ...
        for ipnt in range(self.npnt):
            self.grd_sc_points[ipnt], self.grd_sc_normals[ipnt] = (
                self.secondary_qps.find_reflection_point(
                    self.grd_pm_points[ipnt],
                    self.grd_pm_reflections[ipnt],
                    self.wavelength,
                )
            )

        self.grd_sc_reflections = reflect_light(
            self.grd_pm_reflections, self.grd_sc_normals
        )
    ...
    def detect_rays(self):
        ...
        self.grd_length_sc_to_horn = generalized_dot(
            (horn_position - self.grd_sc_points), horn_orientation
        ) / generalized_dot(self.grd_sc_reflections, horn_orientation)
        self.grd_horn_intercept = (
            self.grd_sc_points
            + self.grd_length_sc_to_horn[..., np.newaxis] * self.grd_sc_reflections
        )
        distance_to_horn_center = generalized_norm(
            self.grd_horn_intercept - horn_position
        )
    ...
    def compute_phase(self, phase_offset=0):
        ...
        self.grd_total_path = np.where(
            np.isnan(self.grd_horn_intercept[:, 0]),
            np.nan,
    ...
    def plot_key(self, key, colormap="viridis", zlim=None, dpi=300):
        ...
        if regridded.ndim == 3:  # [nx,ny, 2|3]
            ...
            simple_imshow_map_plot(
                ax, fig, self.x_axis, self.y_axis, regridded, "", cmap, zlim
            )
    ...
    def plot_3d_ray(self, choosen_ray=None, display=True, dpi=300, n_rays=None):
        ...
        ax.scatter(
            self.grd_pm_points[:, 0],
            self.grd_pm_points[:, 1],
            self.grd_pm_points[:, 2],
            marker=".",
            color="black",
        ...
        ax.legend()
        close_figure(
            fig, f"3D visualization of rays", f"ray_visualization.png", dpi, display
        )
    ...
    def _add_3d_ray_to_ax(self, ax, i_ray, style="--", plot_normals=False):
        ...
        if plot_normals:
            ax.quiver(
                *pm_point,
                *self.grd_pm_normals[i_ray],
                label=f"Primary normal",
                color="black",
            )
            if not np.isnan(sc_point[0]):
                ax.quiver(
                    *sc_point,
                    *self.grd_sc_normals[i_ray],
                    label=f"Secondary normal",
                    color="yellow",
                )
        return
    ...
    def plot_detailed_single_ray(self, chosen_ray, dpi=300, display=True):
        ...
        close_figure(
            fig,
            f"Detailed visualization of rays",
            f"detailed_{chosen_ray}_ray_vis.png",
            dpi,
            display,
        )
        return
