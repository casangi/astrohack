...
def convolution_gridding(grid_type, visibilities, weights, lmaxis, diameter, freq, sky_cell_size, grid_size):
    ...
    beam, wei = _convolution_gridding_jit(visibilities, lmaxis, weights, sky_cell_size, laxis, maxis, beam_size)
...
def _convolution_gridding_jit(visibilities, lmaxis, weights, sky_cell_size, laxis, maxis, beam_size):
    ntime, nchan, npol = visibilities.shape

    exponent = 2
    l_kernel = _create_exponential_kernel(beam_size, exponent, sky_cell_size[0])
    m_kernel = _create_exponential_kernel(beam_size, exponent, sky_cell_size[1])
...
@njit(cache=False, nogil=True)
def _create_exponential_kernel(beam_size, exponent, cell_size):
    smoothing = beam_size
    support = 4*smoothing
