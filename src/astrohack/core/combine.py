...
def process_combine_chunk(combine_chunk_params):
    ...
    nddi = len(ddi_list)
    out_xds_name = "/".join(
        [combine_chunk_params["combine_name"], antenna, ddi_list[0]]
    )
    if nddi == 0:
        ...
        logger.info(
            f"{antenna} has a single ddi to be combined, data copied from input file"
        )
        out_xds = ddi_dict[ddi_list[0]]
        out_xds.to_zarr(out_xds_name, mode="w")
    else:
        out_xds = load_image_xds(
            combine_chunk_params["image_name"], antenna, ddi_list[0], dask_load=False
        )
        nddi = len(ddi_list)
        ...
        for ipol in range(npol):
            ...
            if combine_chunk_params["weighted"]:
                pha_sum[ipol, :] = (
                    out_xds["CORRECTED_PHASE"].values[0, 0, ipol, :, :].ravel()
                    * amp_sum[ipol, :]
                )
            else:
                pha_sum[ipol, :] = (
        ...
        u, v = np.meshgrid(
            out_xds.u_prime.values * wavelength, out_xds.v_prime.values * wavelength
        )
        dest_u_axis = u.ravel()
        dest_v_axis = v.ravel()
        for iddi in range(1, nddi):
            logger.info(f"Regridding {antenna} {ddi_list[iddi]}")
            this_xds = load_image_xds(
                combine_chunk_params["image_name"],
                antenna,
                ddi_list[iddi],
                dask_load=False,
            )
            ...
            u, v = np.meshgrid(
                this_xds.u_prime.values * wavelength,
                this_xds.v_prime.values * wavelength,
            )
            loca_u_axis = u.ravel()
            loca_v_axis = v.ravel()
            for ipol in range(npol):
